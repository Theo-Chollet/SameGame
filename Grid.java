 ...
 * @version 1.0
 */
public class Grid implements GridAPI {
    ...
    /**
     * Retourne un bloc aléatoire.
     * @return Les coordonnées col/row.
     */
    public int[] getRandomBlock() {
        ...
        // Calcule les groupes de la grille
        this.calculateGroups();

        // Calcul d'un index aléatoire entre 0 et le nombre de groupes
        groupIdx = rand.nextInt(this.tabGroups.size());

        // Groupe aléatoire sélectionné
        ArrayList<Integer> randGroup = this.tabGroups.get(groupIdx);

        // Calcul d'un index aléatoire entre 0 et le nombre de blocs du groupe
        blockIdx = rand.nextInt(randGroup.size());

        // Bloc aléatoire
        int randBlock = randGroup.get(blockIdx);

        // Index de colonne / ligne
        int col = this.getCol(randBlock);
        int row = this.getRow(randBlock);
    ...
    /**
     * Retourne le bloc d'un groupe au meilleur score : ie le groupe avec le
     * plus de blocs.
     * @return  Les coordonnées col/row.
     */
    public int[] getBestGroup() {
        ...
        // Nombre de blocs dans le groupe
        int max_blocks = 0;
        // Index du groupe maximal
        int max_idx = 0;

        // Parcours de tous les groupes
        for (int groupIdx = 0; groupIdx < this.tabGroups.size(); groupIdx++) {
            // Nouveau maximum ?
            if (max_blocks < this.tabGroups.get(groupIdx).size()) {
                max_blocks = this.tabGroups.get(groupIdx).size();
                max_idx = groupIdx;
            }
        }

        // Retourne les coordonnées d'un bloc du groupe
        // Index de colonne / ligne
        int col = this.getCol(this.tabGroups.get(max_idx).get(0));
        int row = this.getRow(this.tabGroups.get(max_idx).get(0));
    ...
    /**
     * Efface un groupe dont le bloc avec les coordonnée [row, col].
     * @param row
     * @param col
     */
    public void deleteGroup(int row, int col) {
        // Sélectionne le groupe concerné par les coordonnées
        ArrayList<Integer> group = this.getBlockGroup(row, col);
    ...
    public boolean estVide(int x, int y) {
        // Retourne l'état
        return this.isEmpty(getIndex(x, y));
    }
     ...
     * @return Le gain.
     */
    public int nbPoints(int row, int col) {
        ...
        if (group.size() > 1) {
            ...
            gain = (group.size() - 2) * (group.size() - 2);
        }
    ...
    /**
     * Règle le score total.
     */
    public void setScore(int score) {
        // Mise à jour du score stocké dans la grille
    ...
    /**
     * Retourne la taille de la grille en X (lignes).
     * @return La taille de la grille en X (lignes).
     */
    public int getSizeX() {
        return Defines.NB_ROWS;
    }

    /**
     * Retourne la taille de la grille en Y (colonnes).
     * @return La taille de la grille en Y (colonnes).
     */
    public int getSizeY() {
        return Defines.NB_COLS;
    }
}