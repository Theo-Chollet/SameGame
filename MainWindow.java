...
public class MainWindow extends JFrame {
    ...
    /** Lance le bot aléatoire
     *
     */
    public void startRandomBot() {
        // Création du bot
        RandomBot bot = new RandomBot();

        // Récupération des coordonnées du bloc à jouer
        int[] block = bot.playMove(this.grid);

        // Vérification de suppression
        this.checkBlock(block[0], block[1]);
    }

    /** Lance le bot glouton
     *
     */
    public void startGreedyBot() {
        // Création du bot
    ...
    /** Méthode qui vérifie si le bloc cliqué doit faire disparaitre un groupe
     * @param row Index de ligne du bloc.
     * @param col Index de coilonne du bloc.
     */
    public void checkBlock(int row, int col) {
        // Recherche du groupe de blocs concerné
        ArrayList<Integer> group = this.grid.getBlockGroup(row, col);

        // Récupération de l'index de bloc
        int blockIdx = this.grid.getIndex(row, col);

        // Vérifie si le groupe n'est pas nul (signifie qu'il reste que des blocs
        // sans voisin) et si la case n'est pas vide
        if (group != null && !(this.grid.isEmpty(blockIdx))) {
            // Mise à jour du score
            this.scorePanel.setScore(group.size());

            // 2éme fois car score aussi stocké dans la grille (cf. interface)
            this.grid.setScore(group.size());
